...
export default function OfflineSyncBanner() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+64px)] z-40 flex justify-center px-4 sm:bottom-4">
      <div
        className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-medium shadow-lg backdrop-blur ${
          queue.failed > 0
      ...
      >
        <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${!online ? "bg-amber-500" : queue.failed > 0 ? "bg-red-500" : "bg-emerald-500"}`} />
        {label}
      </div>
    </div>
  );
