...
// A ranked list styled like an avatar leaderboard (initial badge, name, a
// slim progress bar, amount) -- distinct from the plain bar-list used by
// ListStatWidget elsewhere, to give income sources their own identity.
export default function IncomeSourcesRankedWidget({ title, items }: { title: string; items: IncomeSourceItem[] }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-card border border-emerald-200/70 bg-gradient-to-br from-emerald-50 via-surface to-surface p-4 dark:border-emerald-900/50 dark:from-emerald-950/40 dark:via-surface dark:to-surface">
      <div className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-emerald-400/20 blur-2xl dark:bg-emerald-500/10" />

      <p className="relative mb-3 text-xs font-semibold uppercase tracking-wide text-emerald-700/80 dark:text-emerald-300/80">
        {title}
      </p>

      {items.length === 0 ? (
        <p className="relative text-sm text-surface-foreground-soft">No income yet.</p>
      ) : (
            ...
            <div key={item.label} className="flex items-center gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="shrink-0 text-emerald-700 dark:text-emerald-300">{item.displayValue}</span>
                </div>
