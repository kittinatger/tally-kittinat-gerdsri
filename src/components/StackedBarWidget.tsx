  ...
  return (
    <WidgetCard color="slate">
      ...
      {total <= 0 ? (
        ...
        <>
          <div className="flex h-4 w-full gap-0.5 overflow-hidden rounded-full bg-bg-soft">
            {segments.map((s) => (
              <div
                key={s.label}
          ...
          <div className="mt-3.5 flex flex-wrap gap-x-3 gap-y-2">
            {segments.slice(0, 6).map((s) => (
              <span key={s.label} className="flex items-center gap-1.5 text-xs">
                <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${s.colorClassName}`} />
                <span className="text-surface-foreground-soft">
                  {s.label} <span className="font-medium text-surface-foreground">{s.displayValue}</span>
                </span>
              </span>
            ))}
          </div>
