...
// retry vs. just try again). Also logs to the local error log (see
// lib/error-log.ts) so Settings > Error reports has something to show —
// `context` names the action that failed (e.g. "Scan document"); left at
// its generic default for call sites that don't pass one.
export function describeFetchError(err: unknown, context = "Action"): string {
  const message = (() => {
