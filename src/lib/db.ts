...
function ensureSchema(): Promise<void> {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // Which network to force the wallet itself onto — same idea as
      // force_currency (a real card's network is inherent to the artwork,
      // e.g. a Visa transit card's design already shows a Visa mark, so
      // letting the picker choose a different network makes no sense).
      ...
      // Defaults to 'top', matching where it always rendered, so nothing
      // existing changes until someone picks otherwise.
      await sql`ALTER TABLE wallets ADD COLUMN IF NOT EXISTS card_number_position TEXT NOT NULL DEFAULT 'top';`;
      // When true, shows just the bare last4 digits instead of the full
      // masked "•••• •••• •••• 1234" row.
      ...
      // User-named custom fields on a pass — {key: label} JSON text, same
      // convention as membership_cards.fields. Lets the pass editor's
      // custom field placement offer more than just the current
      // template's fixed field set (previously the only way to add a
      // field there was picking one of the template's own predefined
      // ones — there was no way to type a field of your own).
      await sql`ALTER TABLE membership_cards ADD COLUMN IF NOT EXISTS custom_field_labels TEXT NOT NULL DEFAULT '{}';`;

      // Independent visibility toggles for the header's logo/icon avatar
      // and name text — a logo asset that already spells out the brand
      // (or a banner that does) can make repeating it in the header feel
      // redundant, so either can be hidden without deleting the
      // underlying logo image or the card's own name.
      ...
      // Whether the human-readable code value shows alongside the
      // QR/barcode itself — a purely cosmetic toggle (the code still
      // scans exactly the same either way); see MembershipCardCode's
      ...
      // The card-look wallet visual can now show a contactless/NFC symbol,
      // same corner-badge treatment as the network logo. DEFAULT false
      // here so every existing card is unaffected by this migration —
      // createWallet's own `input.showNfc ?? true` is what actually makes
      // it show up by default, but only for a wallet created *after* this
      // shipped (its INSERT always states the column explicitly, so this
      // DEFAULT never applies to it). Same idea for card_templates'
      // force_show_nfc: an existing template's author never opted into
      // forcing anything about a symbol that didn't exist yet.
      await sql`ALTER TABLE wallets ADD COLUMN IF NOT EXISTS show_nfc BOOLEAN NOT NULL DEFAULT false;`;
      await sql`ALTER TABLE card_templates ADD COLUMN IF NOT EXISTS force_show_nfc BOOLEAN;`;

      // The NFC symbol gets its own corner selector, independent of
      // badge_position — same "topLeft" default WalletCardShape's own
      // DEFAULT_NFC_POSITION falls back to (the opposite corner from the
      // badge's own default), so an existing wallet that later turns NFC
      // on doesn't have it land in the same spot as its badge by surprise.
      ...
      // Which spot to force the masked card-number row into — same idea
      // as force_chip_position, only meaningful for templates that also
...
export type WalletRow = {
  ...
  /** Whether the contactless/NFC symbol renders — defaulting false so an
   * existing wallet stays unaffected (see the show_nfc migration comment). */
  show_nfc: boolean;
  ...
  /** Where the masked card-number row renders — see card-number-position.ts. */
...
export async function createWallet(
  ...
  // New (not editing an existing wallet), so unlike the DB column's own
  // false default, a freshly created wallet shows the NFC symbol unless
...
export type CardTemplateRow = {
  ...
  // Whether to force the contactless/NFC symbol shown or hidden — same
  // three-state convention as the other force_show_* columns. NULL (the
  // only possible value for any template submitted before this existed)
  // means "don't touch it".
  force_show_nfc: boolean | null;
  // Which corner to force the NFC symbol into — independent of
  ...
  // Which spot to force the masked card-number row into — same idea as
  // force_chip_position, only meaningful when the card number itself is
  // shown (force_show_card_number !== false). NULL means "don't touch it".
  force_card_number_position: string | null;
...
export async function createCardTemplate(
  ...
  input: {
    ...
    forceCardNumberPosition?: string | null;
  },
  ...
  const { rows } = await sql.query<CardTemplateRow>(
    `INSERT INTO card_templates (
       ...
       country, force_name_position, lock_text_color, category, force_network, force_show_holder_name, force_show_expiry, force_show_nfc, force_nfc_position, force_nfc_size, lock_svg_colors, force_chip_position, force_card_number_position, status
     )
    ...
    [
      ...
      input.forceCardNumberPosition ?? null,
    ],
  );
  return rows[0];
}
...
export async function updateCardTemplate(
  ...
  const newForceCardNumberPosition =
    input.forceCardNumberPosition !== undefined ? input.forceCardNumberPosition : existing.force_card_number_position;
  ...
  const { rows } = await sql.query<CardTemplateRow>(
    `UPDATE card_templates
     SET name = $1, color = $2, background = $3, text_color = $4,
         ...
         force_currency = $11, country = $12, force_name_position = $13, lock_text_color = $14, category = $15, force_network = $16, force_show_holder_name = $17, force_show_expiry = $18, force_show_nfc = $19, force_nfc_position = $20, force_nfc_size = $21, lock_svg_colors = $22, force_chip_position = $23, force_card_number_position = $24, status = $25, reviewed_at = ${bumpReviewedAt ? "now()" : "reviewed_at"}
     WHERE id = $26
     RETURNING ${CARD_TEMPLATE_COLUMNS}, NULL AS submitted_by_username;`,
    [
      newName,
...
export type MembershipCardRow = {
  ...
  has_banner: boolean;
  category: string;
  /** Raw JSON text ({key: label}) — see normalizeCustomFieldLabels in membership-templates.ts. */
...
export async function createMembershipCard(
  ...
  const showCodeText = input.showCodeText ?? true;
  const { rows } = await sql<MembershipCardRow>`
